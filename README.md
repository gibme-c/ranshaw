...
The precomputed ECFFT data (isogeny chain coefficients and domain cosets) is generated by the `ranshaw-gen-ecfft` tool and checked into the repository as `.inl` files. Both fields currently use 16-level domains (65,536 evaluation points).
...
Ran has ~107 bits of twist security; Shaw has ~99 bits. **Every externally-received point must pass on-curve validation** via the `frombytes` functions, which return an error for off-curve points.
